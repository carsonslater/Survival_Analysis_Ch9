...
\begin{itemize}
\item In the preceding sections, a range of models for univariate survival data and several parametric models were presented, which facilitate the investigation of the effects of covariates on survival outcomes.
\item This chapter focuses on graphical checks of the appropriateness of these models, rather than relying on formal statistical tests of lack of fit.
...
\item The graphical models discussed in this context serve as a paradigm for identifying and rejecting clearly inappropriate models, rather than providing conclusive evidence for the correctness of a particular parametric model. We prioritize graphical methods over formal statistical tests due to their limitations in detecting model misspecification, particularly in small samples or large datasets. Graphical checks serve as a means to identify clearly inadequate models, rather than verifying the correctness of specific parametric models.
\end{itemize}
...
\item For example, consider the log logistic distribution. In this case
...
Waiting Time to Transplant (KM pp. 360-362)}} Consider the data presented in
Section 1.10 of KM12 comparing allogeneic (Allo) bone marrow transplants from an HLA-matched
sibling or an autogeneic (Auto) bone marrow transplant for patients with either Hodgkin’s disease
(HL) or non-Hodgkin’s lymphoma (NHL). Below are plots of the
...
of the figures that we generated versus figures 12.1-4 in the book
shows that they did in fact do this. In our figures, we elected to keep the extreme $t$ values.}
\end{itemize}
...
\caption[Diagnostic plots for the Allo survival data (gold) plotted along with the Auto survival data (green)]{Diagnostic plots for the Allo survival data (gold) plotted along with the Auto survival data (green).}\label{fig:unnamed-chunk-1}
\end{figure}
...
\begin{align*}
t_{kp} & =S_{k}^{-1}(1-p), \quad k \in (0, 1).
\end{align*}
...
two groups, estimating the percentiles $t_{0p}$, $t_{1p}$ for various
values of $p$, and then plotting the estimated percentiles in the first group against the estimated percentiles the second group. If the assumption
holds, the graph should be a straight line with slope $\theta$.
\end{itemize}

\noindent\textit{\textbf{Example:
Waiting Time to Transplant (Continued)}} Again consider the Allo-Auto dataset. A Kaplan-Meier estimate
...
\begin{itemize}
\item In equation (7.1.2), Cox–Snell residual is defined as $r_{j}=\widehat{H}(T_{j}|\mathbf{Z}_{j})$, where $\widehat{H}$
...
\hline 
Exponential &
$\hat{\lambda}t_{i}\exp\left\{ \boldsymbol{\hat{\beta'}}\boldsymbol{Z}_{i}\right\} $\tabularnewline
\hline 
Weibull &
$\hat{\lambda}\exp\left\{ \boldsymbol{\hat{\beta'}}\mathbf{Z}_{i}\right\} t_{1}^{\hat{\alpha}}$\tabularnewline
\hline 
Log logistic &
$\log\left[\frac{1}{1+\hat{\lambda}\exp\left\{ \hat{\boldsymbol{\beta'}}\mathbf{Z}_{i}\right\} t_{i}^{\hat{\alpha}}}\right]$\tabularnewline
\hline 
Log normal &
$\log\left[1-\Phi\left(\frac{\log T_{i}-\boldsymbol{\hat{\mu}}-\boldsymbol{\hat{\gamma}'}\mathbf{Z}_{i}}{\hat{\sigma}}\right)\right]$\tabularnewline
\hline 
\end{tabular}
...
\begin{align*}
s_{j} & =\frac{\log T_{j}-\hat{\mu}-\boldsymbol{\hat{\gamma}'}\mathbf{Z}_{j}}{\widehat{\sigma}}
\end{align*}


\end{itemize}

\noindent\textit{\textbf{Example: Regression for Laryngeal Cancer Data}}

\noindent In Figure 9.2, the cumulative hazard plots for the Cox–Snell residuals are shown for the exponential, Weibull, log logistic and log normal regression models for the laryngeal cancer data. We see from these plots that all four models give reasonable fits to the data, the best being the log normal and log logistic models.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=.25\textwidth]{figures/figure1}\quad
  \includegraphics[width=.25\textwidth]{figures/figure2}\\
  \includegraphics[width=.25\textwidth]{figures/figure3}\quad
  \includegraphics[width=.25\textwidth]{figures/figure4}
  \caption{Cox–Snell residuals to assess the fit of (a) Exponential, (b) Weibull, (c) Log-logistic and (d) Log-normal regression model for the laryngeal cancer dataset.}
  \label{fig:sub1}
\end{figure}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{par}\hldef{(}\hlkwc{mfrow} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{2}\hldef{,} \hlnum{1}\hldef{))}
\hlkwd{plot}\hldef{(}\hlkwd{seq}\hldef{(mresid), mresid,} \hlkwc{xlab} \hldef{=} \hlsng{"time"}\hldef{,} \hlkwc{ylab} \hldef{=} \hlsng{"Martingale Residuals"}\hldef{)}
\hlkwd{plot}\hldef{(}\hlkwd{seq}\hldef{(dresid), dresid,} \hlkwc{xlab} \hldef{=} \hlsng{"time"}\hldef{,} \hlkwc{ylab} \hldef{=} \hlsng{"Deviance Residuals"}\hldef{)}
\end{alltt}
\end{kframe}
