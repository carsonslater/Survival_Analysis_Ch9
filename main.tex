...
\begin{alltt}
\hlcom{# Allo-Auto Data ----------------------------------------------------------}
\hlcom{# Survival Analysis Ch. 9 -------------------------------------------------}

\hlcom{# Packages ----------------------------------------------------------------}
\hlkwd{library}\hldef{(}\hlsng{"tidyverse"}\hldef{)}
\hlkwd{library}\hldef{(}\hlsng{"KMsurv"}\hldef{)}
\hlkwd{library}\hldef{(}\hlsng{"survival"}\hldef{)}
\hlkwd{library}\hldef{(}\hlsng{"scales"}\hldef{,} \hlkwc{warn.conflicts} \hldef{=} \hlnum{FALSE}\hldef{)}
\hlkwd{library}\hldef{(}\hlsng{"patchwork"}\hldef{)}
\hlkwd{library}\hldef{(}\hlsng{"tinytex"}\hldef{)}
\hlkwd{theme_update}\hldef{(}\hlkwc{panel.grid.minor} \hldef{=} \hlkwd{element_blank}\hldef{())}

\hldef{baylor_green} \hlkwb{<-} \hlsng{"steelblue"}
\hldef{baylor_gold} \hlkwb{<-} \hlsng{"firebrick"}

\hldef{aa} \hlkwb{<-} \hlkwd{data}\hldef{(}\hlsng{"alloauto"}\hldef{)}


\hlcom{# Fit the survival model for each group and extract cumulative hazards}
\hldef{km_fit_groups} \hlkwb{<-} \hldef{alloauto |>}
  \hlkwd{group_by}\hldef{(type)} \hlopt{%>%}
  \hlkwd{do}\hldef{(}
    \hlkwc{fit} \hldef{=} \hlkwd{survfit}\hldef{(}\hlkwd{Surv}\hldef{(time, delta)} \hlopt{~} \hlnum{1}\hldef{,} \hlkwc{data} \hldef{= .)}
  \hldef{) |>}
  \hlkwd{mutate}\hldef{(}
    \hlkwc{time} \hldef{=} \hlkwd{list}\hldef{(fit}\hlopt{$}\hldef{time),}
    \hlkwc{cumhaz} \hldef{=} \hlkwd{list}\hldef{(fit}\hlopt{$}\hldef{cumhaz)}
  \hldef{) |>}
  \hlkwd{unnest}\hldef{(}\hlkwd{c}\hldef{(time, cumhaz)) |>}
  \hlkwd{select}\hldef{(type, time, cumhaz) |>} \hlkwd{mutate}\hldef{(}\hlkwc{type} \hldef{=} \hlkwd{as.factor}\hldef{(type))}

\hldef{allo_fit} \hlkwb{<-} \hldef{alloauto} \hlopt{%>%}
  \hldef{dplyr}\hlopt{::}\hlkwd{filter}\hldef{(type} \hlopt{==} \hlnum{1}\hldef{)} \hlopt{%>%}
  \hlkwd{survfit}\hldef{(}\hlkwd{Surv}\hldef{(time, delta)} \hlopt{~} \hlnum{1}\hldef{,} \hlkwc{data} \hldef{= .)}

\hldef{auto_fit} \hlkwb{<-} \hldef{alloauto} \hlopt{%>%}
  \hldef{dplyr}\hlopt{::}\hlkwd{filter}\hldef{(type} \hlopt{==} \hlnum{2}\hldef{)} \hlopt{%>%}
  \hlkwd{survfit}\hldef{(}\hlkwd{Surv}\hldef{(time, delta)} \hlopt{~} \hlnum{1}\hldef{,} \hlkwc{data} \hldef{= .)}

\hldef{tb} \hlkwb{<-} \hlkwd{tibble}\hldef{(}
  \hlsng{"Time"} \hldef{=} \hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(auto_fit}\hlopt{$}\hldef{cumhaz[}\hlopt{-}\hlnum{49}\hldef{]),}
  \hlsng{"Allo"} \hldef{=} \hlkwd{c}\hldef{(allo_fit}\hlopt{$}\hldef{cumhaz,} \hlnum{NA}\hldef{),}
  \hlsng{"Auto"} \hldef{= auto_fit}\hlopt{$}\hldef{cumhaz[}\hlopt{-}\hlnum{49}\hldef{]}
\hldef{) |>} \hlkwd{pivot_longer}\hldef{(}
  \hlkwc{cols} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"Allo"}\hldef{,} \hlsng{"Auto"}\hldef{),}
  \hlkwc{names_to} \hldef{=} \hlsng{"type"}
\hldef{) |>} \hlkwd{mutate}\hldef{(}
  \hlkwc{l_time} \hldef{=} \hlkwd{log}\hldef{(Time),}
  \hlkwc{l_val} \hldef{=} \hlkwd{log}\hldef{(value),}
  \hlkwc{l_norm} \hldef{=} \hlkwd{qnorm}\hldef{(}\hlnum{1} \hlopt{-} \hlkwd{exp}\hldef{(}\hlopt{-}\hldef{value)),}
  \hlkwc{l_logistic} \hldef{=} \hlkwd{log}\hldef{(}\hlkwd{exp}\hldef{(value} \hlopt{-} \hlnum{1}\hldef{))}
\hldef{)}

\hlcom{# 1st Plot Attempt --------------------------------------------------------}
\hlcom{# Figure 12.1}

\hldef{p1} \hlkwb{<-} \hldef{tb |>}
  \hlkwd{ggplot}\hldef{(}\hlkwd{aes}\hldef{(Time, value,} \hlkwc{colour} \hldef{= type))} \hlopt{+}
  \hlkwd{geom_step}\hldef{()} \hlopt{+}
  \hlkwd{scale_color_manual}\hldef{(}\hlkwc{values} \hldef{=} \hlkwd{c}\hldef{(}\hlsng{"firebrick"}\hldef{,} \hlsng{"steelblue"}\hldef{))} \hlopt{+}
  \hlkwd{labs}\hldef{(}\hlkwc{title} \hldef{=} \hlsng{"Exponential"}\hldef{,}
       \hlkwc{x} \hldef{=} \hlsng{"Time"}\hldef{,}
       \hlkwc{y} \hldef{=} \hlsng{"Est. Cumulative Hazard"}\hldef{,}
...
\hldef{p2} \hlkwb{<-} \hldef{tb |>}
  \hlkwd{ggplot}\hldef{(}\hlkwd{aes}\hldef{(l_time, l_val,} \hlkwc{colour} \hldef{= type))} \hlopt{+}
  \hlkwd{geom_step}\hldef{()} \hlopt{+}
  \hlkwd{scale_color_manual}\hldef{(}\hlkwc{values} \hldef{=} \hlkwd{c}\hldef{(baylor_gold, baylor_green))} \hlopt{+}
  \hlkwd{labs}\hldef{(}\hlkwc{title} \hldef{=} \hlsng{"Weibull"}\hldef{,}
       \hlkwc{x} \hldef{=} \hlsng{"Log Time"}\hldef{,}
       \hlkwc{y} \hldef{=} \hlsng{"Log Est. Cumulative Hazard"}\hldef{,}
       \hlkwc{color} \hldef{=} \hlsng{"Group"}\hldef{)}  \hlopt{+}
  \hlkwd{theme_minimal}\hldef{(}\hlkwc{base_size} \hldef{=} \hlnum{10}\hldef{)} \hlopt{+}
  \hlkwd{theme}\hldef{(}\hlkwc{legend.position} \hldef{=} \hlsng{"none"}\hldef{)}
...
\hlkwd{library}\hldef{(KMsurv)}
\hlkwd{library}\hldef{(ggplot2)}
\hlkwd{library}\hldef{(dplyr)}
\hlkwd{theme_set}\hldef{(}\hlkwd{theme_minimal}\hldef{())}

\hlkwd{data}\hldef{(}\hlsng{"alloauto"}\hldef{)}

\hldef{km_fit} \hlkwb{<-} \hlkwd{survfit}\hldef{(}\hlkwd{Surv}\hldef{(time, delta)} \hlopt{~} \hldef{type,} \hlkwc{data} \hldef{= alloauto)}

\hldef{allo_percentile} \hlkwb{<-} \hlkwd{quantile}\hldef{(km_fit[}\hlnum{1}\hldef{],} \hlkwc{probs} \hldef{=} \hlkwd{seq}\hldef{(}\hlnum{.05}\hldef{,} \hlnum{.35}\hldef{,} \hlkwc{by} \hldef{=} \hlnum{.05}\hldef{))}\hlopt{$}\hldef{quantile}
\hldef{auto_percentile} \hlkwb{<-} \hlkwd{quantile}\hldef{(km_fit[}\hlnum{2}\hldef{],} \hlkwc{probs} \hldef{=} \hlkwd{seq}\hldef{(}\hlnum{.05}\hldef{,} \hlnum{.35}\hldef{,} \hlkwc{by} \hldef{=} \hlnum{.05}\hldef{))}\hlopt{$}\hldef{quantile}

\hlkwd{tibble}\hldef{(allo_percentile, auto_percentile) |>}
  \hlkwd{ggplot}\hldef{(}\hlkwd{aes}\hldef{(}\hlkwc{x} \hldef{= auto_percentile,} \hlkwc{y} \hldef{= allo_percentile))} \hlopt{+}
  \hlkwd{geom_line}\hldef{()} \hlopt{+}
  \hlkwd{scale_x_continuous}\hldef{(}\hlkwc{name} \hldef{=} \hlsng{"Estimated Percentile for Auto Group"}\hldef{,}
                     \hlkwc{breaks} \hldef{=} \hlkwd{seq}\hldef{(}\hlnum{0}\hldef{,} \hlnum{12}\hldef{,} \hlkwc{by} \hldef{=} \hlnum{2}\hldef{),}
                     \hlkwc{limits} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0}\hldef{,} \hlnum{12}\hldef{))} \hlopt{+}
  \hlkwd{scale_y_continuous}\hldef{(}\hlkwc{name} \hldef{=} \hlsng{"Estimated Percentile for Allo Group"}\hldef{,}
                     \hlkwc{breaks} \hldef{=} \hlkwd{seq}\hldef{(}\hlnum{0}\hldef{,} \hlnum{12}\hldef{,} \hlkwc{by} \hldef{=} \hlnum{2}\hldef{),}
                     \hlkwc{limits} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0}\hldef{,} \hlnum{12}\hldef{))} \hlopt{+}
  \hlkwd{theme}\hldef{(}
    \hlkwc{axis.line} \hldef{=} \hlkwd{element_line}\hldef{(}
      \hlkwc{colour} \hldef{=} \hlsng{"black"}\hldef{,}
      \hlkwc{size} \hldef{=} \hlnum{.5}\hldef{,}
      \hlkwc{linetype} \hldef{=} \hlsng{"solid"}
    \hldef{),}
    \hlkwc{panel.grid.minor} \hldef{=} \hlkwd{element_blank}\hldef{(),}
    \hlkwc{panel.grid.major} \hldef{=} \hlkwd{element_blank}\hldef{()}
  \hldef{)}

\hlcom{# estimate slope ------------------------------------------------}

\hlcom{# tibble(allo_percentile, auto_percentile) |>}
\hlcom{#  lm(data = _, formula = allo_percentile ~ auto_percentile)}
\hlcom{# (8.88-2.14)/(11.5 - 4.93)}
\hlcom{# (8.88 - .855)/(11.5 - 1.41)}

\hlcom{# Allo has no percentile estimates past 45}
\hlcom{#quantile(km_fit[1], probs = seq(.05, .95, by = .05))$quantile}
\hlcom{#quantile(km_fit[2], probs = seq(.05, .95, by = .05))$quantile}
\end{alltt}
\end{kframe}
\end{knitrout}
