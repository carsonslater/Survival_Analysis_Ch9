...
\noindent Note that only the the range of percentiles $.05$ to $.35$ is actually plotted above, but the portion that is plotted looks roughly linear.
The figure appears to have a slope $q$ between $0.6$ and $0.8$, which is our crude estimate for the acceleration factor $\theta$.
