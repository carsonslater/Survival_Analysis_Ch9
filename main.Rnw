...
<<echo=FALSE, fig.align='center', fig.width=8, fig.height=9, warning=FALSE, fig.cap='Diagnostic plots for the Allo survival data (gold) plotted along with the Auto survival data (green).'>>=
# Allo-Auto Data ----------------------------------------------------------
...
allo_fit <- alloauto %>%
  filter(type == 1) %>%
  survfit(Surv(time, event = delta) ~ 1, data = .)
...
tb2 <- tibble(
  "allo" = c(allo_fit$surv, NA, NA),
  "auto" = auto_fit$surv,
  "x" = seq(from = 0.55, to = 1, length.out = 49),
  "y" = x
) 
tb2 |> 
  ggplot() +
  geom_line(aes(allo, allo)) +
  geom_line(aes(x, y), color = "red") + 
  labs(title = "QQ-Plot for the Allo-Auto Data",
       x = "Allo Survival Quantiles",
       y = "Auto Survival Quantiles") +
  theme_minimal(base_size = 10) +
  theme(legend.position = "none")
@
...
<<>>=
par(mfrow = c(2, 1))
plot(seq(mresid), mresid, xlab = "time", ylab = "Martingale Residuals")
plot(seq(dresid), dresid, xlab = "time", ylab = "Deviance Residuals")
@
